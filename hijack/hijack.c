...
get_syms(int fd, Elf32_Shdr *symh, Elf32_Shdr *strh)
{
	...
	/* sanity */
	if (symh->sh_size % sizeof(Elf32_Sym)) { 			//error is not duiqi
		printf("elf_error\n");
		goto out;
	}

	/* symbol table */
	sl->num = symh->sh_size / sizeof(Elf32_Sym);		//sym table header->size / len of one piece
	sl->sym = (Elf32_Sym *) xmalloc(symh->sh_size);
	rv = pread(fd, sl->sym, symh->sh_size, symh->sh_offset);		//symh sh_offset current piece of section offset
	if (0 > rv) {
		//perror("read");
		goto out;
	}
...
static int
do_load(int fd, symtab_t symtab)		//open libc.so  read symtab
{
	int rv;
	size_t size;
	Elf32_Ehdr ehdr;
	...
	if (strncmp(ELFMAG, ehdr.e_ident, SELFMAG)) { /* sanity */		//elf magic 
		printf("not an elf\n");
		goto out;
	}
	if (sizeof(Elf32_Shdr) != ehdr.e_shentsize) { /* sanity */		//section header size
		printf("elf error\n");
		goto out;
	}
	...
	shdr = (Elf32_Shdr *) xmalloc(size);				//shdr section header content 
	rv = pread(fd, shdr, size, ehdr.e_shoff);			//section header offset   pread?
	if (0 > rv) {
		//perror("read");
		goto out;
	}
	...
	size = shdr[ehdr.e_shstrndx].sh_size;									//size of string section table 
	shstrtab = (char *) xmalloc(size);
	rv = pread(fd, shstrtab, size, shdr[ehdr.e_shstrndx].sh_offset);		//字符串表
	if (0 > rv) {
		//perror("read");
		goto out;
	}
	...
	symh = dynsymh = NULL;
	strh = dynstrh = NULL;
	for (i = 0, p = shdr; i < ehdr.e_shnum; i++, p++)		//lookfor all section to find .dynsym .dynsym .strh .dynstrh
		if (SHT_SYMTAB == p->sh_type) {			//==2
			if (symh) {
				printf("too many symbol tables\n");
				goto out;
			}
			symh = p;
		} else if (SHT_DYNSYM == p->sh_type) {	//11
			if (dynsymh) {
				printf("too many symbol tables\n");
				goto out;
			}
			dynsymh = p;
		} else if (SHT_STRTAB == p->sh_type		//3
	...
	/* symbol tables */
	if (dynsymh)
		symtab->dyn = get_syms(fd, dynsymh, dynstrh);			//get dynamic symbol table according dyn symbol table header and dyn str header
	if (symh)
		symtab->st = get_syms(fd, symh, strh);
	ret = 0;
out:
	free(shstrtab);		//free section str table
	free(shdr);			//free section tables
	return ret;
}

static symtab_t
load_symtab(char *filename)	//    /system/lib/libc.so
{
	int fd;
	symtab_t symtab;		//st_value 如果该符号已定义，则 st_value 保存的是该符号在其所定义的 section (由 st_shndx 指定) 中的偏移量。

	symtab = (symtab_t) xmalloc(sizeof(*symtab));		//xmalloc 
	...
	if (0 > do_load(fd, symtab)) {						//load so file and get symtab and symstr
...
load_memmap(pid_t pid, struct mm *mm, int *nmmp)
{
	...
	p = raw;
	while (1) {						//why?
		rv = read(fd, p, sizeof(raw)-(p-raw));	//strange 为何分多次读取
		if (0 > rv) {
			//perror("read");
			return -1;
		}
	...
	while (p) {
		...
		/* search backward for other mapping with same name */	//the first has exec right
		for (i = nmm-1; i >= 0; i--) {
			m = &mm[i];
	  ...
	  struct mm *mm, int nmm)
{
	...
	for (i = 0, m = mm; i < nmm; i++, m++) {
		...
		if (!strncmp(".so", p, 3) || (p[0] == '-' && isdigit(p[1])))			//magic
			break;
	}
	if (i >= nmm)
		/* not found */
...
static int
lookup2(struct symlist *sl, unsigned char type,		// symlist type=FUNC name=func name  val = offset
...
find_name(pid_t pid, char *name, unsigned long *addr)
{
	...
	if (0 > load_memmap(pid, mm, &nmm)) {	//get mmap列表
		printf("cannot read memory map\n");
		return -1;
	}
	if (0 > find_libc(libc, sizeof(libc), &libcaddr, mm, nmm)) {		//strncpy 拷贝 n个长度，如果不满n 则以NULL填充 ，获取libc的全路径名字， 起始地址 和索引nmm
		printf("cannot find libc\n");
		return -1;
	}
	s = load_symtab(libc);							//load symtab  and return symtab_t
	if (!s) {
		printf("cannot read symbol table\n");
		return -1;
	}
	if (0 > lookup_func_sym(s, name, addr)) {		//get symbol address
...
#define HELPSTR "error usage: %s -p PID -l LIBNAME [-d (debug on)] [-z (zygote)] [-m (no mprotect)] [-s (appname)] [-Z (trace count)] [-D (debug level)] [-i (listener on)]\n"
...
int main(int argc, char *argv[])
{
    ...
    int listen = 0;
 	...
 	while ((opt = getopt(argc, argv, "p:l:dzmsi:Z:D:")) != -1) {		//method in <unistd.h> return int ? switch 'char'
		switch (opt) {
			case 'p':
